            ...
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'pks_client', usernameVariable: 'USERNAME', passwordVariable: 'USERPASS')]) {
                    script {
                        ...
                        def result = sh(script: "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@$pks_client \"kubectl get deployment gocicd &> /dev/null ; echo \$?\"" , returnStdout: true).trim()
                        echo result
                        if (!result) {
